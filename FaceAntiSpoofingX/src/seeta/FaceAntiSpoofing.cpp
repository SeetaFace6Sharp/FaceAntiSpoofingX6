...
#define LIBRARY_VERSION GENERATE_VER( \
    (SEETA_FACE_ANTI_SPOOFING_MAJOR_VERSION)(SEETA_FACE_ANTI_SPOOFING_MINOR_VERSION)(SEETA_FACE_ANTI_SPOOFING_SINOR_VERSION))
...
namespace seeta
{
    namespace v6
    {
        ...
        public:
            ...
            static std::vector<SeetaLabeledBox> NMS(const std::vector<SeetaLabeledBox> &box_list, float threshold)
            {
                ...
                for (size_t i = 0; i < list.size(); ++i)
                {
                    ...
                    {
                        if (IoU(list[i].pos, list[j].pos) > threshold)
                            flag[j] = true;
                    }
                }
            ...
            static std::vector<SeetaLabeledBox> FinalNMS(const std::vector<SeetaLabeledBox> &rect_list, float threshold)
            {
                ...
                for (size_t i = 0; i < list.size(); ++i)
                {
                    ...
                    for (size_t j = i + 1; j < list.size(); ++j)
                    {
                        if (IoM(list[i].pos, list[j].pos) > threshold)
                            flag[j] = true;
                    }
                }
                std::vector<SeetaLabeledBox> result;
        ...
        SeetaLabeledBoxArray BoxDetector::Detect(const SeetaImageData &image) const
        {
            ...
            if (image.data == nullptr || image.channels <= 0 || image.width <= 0 || image.height <= 0)
                return box_array;
            ...
            if (m_impl->stable)
            {
                for (size_t i = 0; i < boxs.size(); i++)
                {
                    for (size_t j = 0; j < pre_boxs.size(); j++)
                    {
                        ...
                        else if (Implement::IoU(boxs[i].pos, pre_boxs[j].pos) > 0.55)
                        {
        ...
        public:
            ...
            {
                if (!image.data || info.width < 9 || info.height < 9)
                    return 0.0;
        ...
        public:
            ...
            Implement(const seeta::ModelSetting &setting)
            {
                ...
                int index = 0;
                while (setting.model[index])
                    models.push_back(setting.model[index++]);
                if (models.empty())
                {
